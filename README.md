...
- **계정·동기화** — Supabase 로그인(Google · Apple · 이메일 매직링크)으로 기기 간 백업/동기화. *(동기화 엔진은 로드맵 참고)*
...
- [ ] 동기화 엔진 (오프라인 큐 · LWW 머지 · 로컬→클라우드 마이그레이션)
- [ ] 네이티브 인증 (Capacitor 딥링크 / Sign in with Apple)
- [ ] 악보 빌더 · 오디오 재생 · 영어(EN) 지원

### 상태

개인 학습/제품화 프로젝트로 **활발히 개발 중**입니다. 현재 웹 MVP 완료, 백엔드(계정·동기화) 단계 진행 중.
